...
| Tests/eval | MLflow eval and agent tests | API/service tests first; local eval harness before managed MLflow | Implemented: backend tests, visits -> store -> alerts -> feedback -> audit smoke path, Playwright workbench smoke, local OSA eval harness with optional required-provider gate, MLflow-ready artifact export, dry-run MLflow handoff manifest, pilot readiness report with shared activation target blockers, readiness bundle, frontend build verification, summary provider unit coverage |
