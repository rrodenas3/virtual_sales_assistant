...
- Public-safe pilot status snapshots are generated by `scripts/pilot_status_snapshot.py`, summarizing readiness targets, API contract validity, MCP coverage, runtime commands, activation evidence, and next blockers.
- Local handoff and validation-suite artifacts now embed the pilot status snapshot, so every operator handoff contains the compact readiness/API/evidence summary without a separate manual command.
- Local handoff now reuses its prebuilt readiness bundle and API contract when generating the embedded pilot status snapshot, avoiding duplicate local validation work while keeping standalone snapshot generation available.
- Public-safe pilot gap reports are generated by `scripts/pilot_gap_report.py` and embedded in local handoff artifacts, mapping activation blockers to owner hints, next validation commands, and remaining roadmap gates without exposing secrets or client data.
- Pilot gap report logic now lives in the shared governance layer and is exposed through `/integrations/pilot-gap-report`, with manager/admin readiness panels rendering owner hints and next validation commands from the same source as CLI artifacts.
- Public-safe activation runbooks are generated by `scripts/pilot_activation_runbook.py`, embedded in local handoff artifacts, exposed through `/integrations/activation-runbook`, and rendered in manager/admin readiness panels as the step-by-step path from the scaffold to the final VSA pilot outcome.
- README/API architecture alignment is now guarded by `scripts/readme_contract_guard.py`, CI, and local verification so the public architecture front door cannot drift from activation endpoints, linked assets, and required SDD docs.
- Public-safe client discovery packets are generated by `scripts/discovery_packet.py`, embedded in local handoff artifacts, exposed through `/integrations/discovery-packet`, and rendered in manager/admin readiness panels as owner-grouped activation questions without exposing actual values.
- `/agent/run` now includes a deterministic Supervisor/Action Agent SSE path for OSA summaries, order drafts, CRM visit-log drafts, and manager tasks. All action intents call the same service layer as REST routes, order actions stop at draft creation, and human approval remains required before sandbox submit.
- The rep workbench now exposes `/agent/run` controls for summary, HITL order-draft, and CRM visit-log draft intents, with the SSE timeline rendering supervisor decisions, action results, HITL requirements, and audit IDs.
- AI-demo activation now has a single public-safe pack generated by `scripts/ai_demo_activation_pack.py`, embedded in local handoff artifacts, exposed through `/integrations/ai-demo-activation-pack`, and rendered in manager/admin readiness panels. It reports config presence, required commands, evidence artifacts, env evidence keys, and blockers without exposing token values.
