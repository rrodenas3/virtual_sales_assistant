...
from scripts.validate_live_data_contracts import build_manifest_payload, readiness_env_manifest, write_manifest_artifacts  # noqa: E402


def _next_blocking_actions(readiness: dict[str, Any], commands: list[dict[str, str]]) -> list[str]:
    actions: list[str] = []
    for target in readiness.get("activation_targets", []):
        if not isinstance(target, dict) or target.get("ready"):
            continue
        blockers = target.get("blockers", [])
        blocker_text = "; ".join(str(blocker) for blocker in blockers[:3]) or "blocked"
        actions.append(f"{target.get('target')}: {blocker_text}")
    if not actions:
        actions.append("No activation target blockers reported")
    if commands:
        actions.append(f"Next command: {commands[0]['command']}")
    return actions
...
def build_bundle(target: str) -> dict[str, Any]:
    ...
    return {
        ...
        "handoff_summary": {
            "target": target,
            "passed": readiness["passed"] and mcp_smoke["passed"],
            "next_blocking_actions": _next_blocking_actions(readiness, commands),
        },
...
def write_artifacts(bundle: dict[str, Any], output_dir: Path) -> None:
    ...
    write_manifest_artifacts(bundle["live_data_contract_manifest"], contracts_dir)
    ...
    lines.extend(f"- {action}" for action in bundle["handoff_summary"]["next_blocking_actions"])
    lines.extend(
        [
            "",
            "## Components",
